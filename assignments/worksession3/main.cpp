...
void initLights()
{
	for (int i = 0; i < lightEdgeNum; i++)
	{
		for (int j = 0; j < lightEdgeNum; j++)
		{
			...
			lights[(i * lightEdgeNum) + j].lightColor = glm::vec3(1.0);
		}
	}
...
void drawLighting(ew::Shader& _shader)
{
	...
	_shader.setFloat("_Material.Metallic", material.Metallic);
	_shader.setFloat("_Material.Roughness", material.Roughness);
...
void drawGeometry(ew::Shader& _shader, ew::Model& _model, ew::Transform& _modelTransform, GLuint& _texture)
{
	...
	_shader.setMat4("_TransformModel", _modelTransform.modelMatrix());
	_model.draw();
